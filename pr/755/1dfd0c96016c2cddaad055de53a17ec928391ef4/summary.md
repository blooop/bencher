| Metric | Value |
|--------|-------|
| Total tests | 1194 |
| Total time | 102.55s |
| Mean | 0.0859s |
| Median | 0.0010s |

<details>
<summary>Top 10 slowest tests</summary>

| Test | Time (s) |
|------|----------|
| `test.test_bench_examples.TestBenchExamples::test_example_meta` | 20.761 |
| `test.test_over_time_save_perf::test_save_faster_without_aggregated_tab` | 5.104 |
| `test.test_hash_persistent.TestCrossProcessDeterminism::test_hash_stable_across_two_processes[ResultBool]` | 3.780 |
| `test.test_generated_examples::test_generated_example[cartesian_animation/example_cartesian_animation.py]` | 3.061 |
| `test.test_over_time_repeats.TestMaxSliderPoints::test_default_subsampling_caps_at_max` | 3.035 |
| `test.test_generated_examples::test_generated_example[result_types/result_image/example_result_image_to_video.py]` | 2.741 |
| `test.test_bencher.TestBencher::test_combinations_over_time` | 1.480 |
| `test.test_optuna_result.TestOptunaResult::test_collect_optuna_plots_with_repeats` | 1.035 |
| `test.test_over_time_repeats.TestMaxSliderPoints::test_no_subsampling_when_below_default_max` | 0.922 |
| `test.test_bench_runner_copy.TestRunCfgMutationSafety::test_run_does_not_mutate_self_run_cfg` | 0.886 |

</details>