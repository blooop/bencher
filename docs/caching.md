...
|---|---|---|
| Flag | `cache.samples` | `cache.results` |
| Granularity | one entry per call to the benchmark function | one entry per completed sweep |
| Key | the function's inputs plus the run tag — **nothing else** | `BenchCfg.hash_persistent()` — bench name, input/result/const vars, tag, `time.over_time`, `execution.repeats` |
| Written | as each sample finishes | when the sweep completes (unconditionally, see below) |
| Read | only when `cache.samples` is `True` | only when `cache.results` is `True` |
...
function can crash, or the sweep is long enough that you might interrupt it, `cache.samples`
...
> **`cache.only_hash_tag` is a dead flag — do not rely on it.** Its parameter docstring used to
...
The cache flags live on `BenchRunCfg`'s `cache` sub-config (a `CacheCfg`), reached as
`run_cfg.cache.<name>`; `only_plot` lives on the `execution` sub-config, and `run_tag`
stays a top-level `BenchRunCfg` parameter. Each row below was checked against the code
that reads it; the "Reader" column names that site.

| Parameter | Default | What it actually does | Reader |
|---|---|---|---|
| `cache.samples` | `False` | Creates the sample cache at all. When `False` no `Cache` object is constructed, so nothing is stored *or* loaded per sample. | `sweep_executor.py` -> `FutureCache.__init__` (`job.py`) |
| `cache.results` | `False` | Gates the **read** of the benchmark-level cache. On a hit the whole sweep is skipped. The write is not gated — every completed sweep is stored regardless. | `bencher.py` (`elif run_cfg.cache.results:`) |
| `cache.clear` | `False` | Deletes this benchmark's entry from the benchmark-level cache before the run, and skips the cache check for it. | `bencher.py` (`if run_cfg.cache.clear:`) |
| `cache.clear_samples` | `False` | Clears the sample cache entries carrying this benchmark's tag, before sampling starts. Other tags are untouched. | `bencher.py` (`if bench_cfg.cache.clear_samples:`) |
| `cache.overwrite_samples` | `False` | Sets `FutureCache.overwrite`, which makes `submit()` skip the cache lookup and recompute, overwriting the stored value. | `sweep_executor.py` -> `FutureCache.submit` |
| `execution.only_plot` | `False` | Turns a benchmark-level cache miss into `FileNotFoundError` instead of running the sweep. | `bencher.py` |
| `cache.size_mb` | `None` | Maximum disk cache size in **megabytes**; applied as the `size_limit` of every store. `None` uses the 100 GB default. | `bencher.py` -> `SweepExecutor` / `ResultCollector` |
...
| `cache.only_hash_tag` | `False` | **Nothing.** No reader; see the box above. | none |

Two naming traps worth knowing:

- `BenchRunCfg`'s `cache.results` attribute (benchmark-level cache) and the
  `cache_results=` keyword accepted by `BenchRunner.run()` / `bn.run()` are **different
  things**. The keyword is a deprecated alias for the `cache_samples=` keyword and emits a
  `DeprecationWarning`; the attribute is the benchmark-level cache flag. Prefer setting
  attributes on a `BenchRunCfg` explicitly.
- `bn.run()` and `BenchRunner.run()` auto-enable the sample cache for *progressive* runs
...
relevant cache (`cache.clear` / `cache.clear_samples`) or force recomputation with
`cache.overwrite_samples` whenever the measurement itself changes. The `cache.samples`
...
The first run starts from a clean slate (`cache.clear_samples = True`) and dies at
`input_val > 1`, but the samples it *did* compute are already in
`cachedir/sample_cache`. The second run keeps the cache (`cache.clear_samples = False`), loads
...
Do not use `cache.results` for results that are file paths (`ResultImage`, `ResultVideo`,
`ResultPath`). A cached path can outlive the file it names. Set
`run_cfg.cache.results = False` and write to a unique path per parameter combination — see
