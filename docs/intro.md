...
```python
bn.run(SimpleFloat, fidelity=3, repeats=10)
```
...
```python
bn.run(MyBench, fidelity=3, repeats=3, over_time=True)
```
...
```python
bn.run(MyBench, fidelity=3, repeats=3, optimise=30)
```
...
bn.run(MyBench, fidelity=3, optimise=30)
```
