...
class BarResult(HoloviewResult):
    ...
    def _make_bar_fig(self, dataset, result_var, x_dim, by, title, **_kwargs):
        ...
        # Normalise by to a list for uniform handling
        by_dims = [by] if isinstance(by, str) else (by or [])
        by_dims = [d for d in by_dims if d in dataset.dims]

        if by_dims:
            group_coords = [dataset.coords[d].values for d in by_dims]
            for ci, combo in enumerate(iterproduct(*group_coords)):
                sel = dict(zip(by_dims, combo))
                ds_sel = dataset.sel(**sel)
