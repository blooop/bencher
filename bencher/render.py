...
def load_result(path: str | Path) -> BenchResult:
    ...
    Raises:
        RuntimeError: If the pickle cannot be reconstructed. The most likely
            cause is a file saved before the bench_cfg split (holobench 2.0),
            whose flat ``BenchCfg`` layout does not load into the nested class
            shape; unlike the managed cache there is no version stamp here to
            discard it cleanly, so the failure is diagnosed instead. Re-run
            ``collect``/``save_result`` to regenerate the file.
    """
    with Path(path).open("rb") as fh:
        try:
            ...
            raise RuntimeError(
                f"Could not unpickle {path}. If this file was saved by an older "
                "holobench, it predates the bench_cfg split (holobench 2.0), which "
                "changed the pickled class layout; saved bench data from before the "
                "split cannot be loaded and must be regenerated by re-running "
                "collect/save_result."
            ) from exc
