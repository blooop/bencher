...
class Bench(BenchPlotServer):
    ...
    def plot_sweep(
        ...
        elif run_cfg.fidelity > 0:
            inputs = []
            logging.debug("Input vars prior to fidelity adjustment: %s", input_vars_in)
            if len(input_vars_in) > 0:
                for i in input_vars_in:
                    inputs.append(i.with_fidelity(run_cfg.fidelity))
                input_vars_in = inputs
                logging.info(
                    "fidelity=%d → %d samples per variable",
                    run_cfg.fidelity,
                    BenchRunCfg.fidelity_to_samples(run_cfg.fidelity),
                )
