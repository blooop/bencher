...
class CartesianAnimationSweep(bn.ParametrizedSweep):
    ...
    def __call__(self, **kwargs):
        ...
        cfg = CartesianProductCfg(all_vars=sweep_vars, result_names=["result"])

        tag = f"{self.spatial_dims}d_r{self.repeats}_t{self.time_steps}"
...
def example_cartesian_grid(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    ...
    bench.plot_sweep(
        ...
        input_vars=["spatial_dims",bn.sweep("repeats",max_level=3),bn.sweep( "time_steps",max_level=3) ],
    )
