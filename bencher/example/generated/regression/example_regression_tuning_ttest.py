"""Auto-generated example: Regression detection tuning — 'ttest' method (regression_magnitude × alpha)."""

import bencher as bn


class TtestTuning(bn.ParametrizedSweep):
    """Sweep ``regression_magnitude`` × ``alpha`` for ``ttest``.
    ...
    alpha = bn.FloatSweep(
        ...
        bounds=[0.01, 0.2],
        samples=6,
        doc="Detector threshold — significance level for Welch's t-test.",
    )
    ...
    def benchmark(self):
        ...
        from bencher.regression import detect_ttest

        hits = 0
        for trial in range(self._n_trials):
            seed = trial * 1_000_003 + int(self.regression_magnitude * 997) + int(self.alpha * 101)
            ...
            result = detect_ttest(
                "metric", hist_samples, current, alpha=self.alpha, direction=bn.OptDir.minimize
            )
            if result.regressed:
                hits += 1
...
def example_regression_tuning_ttest(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    """Regression detection tuning — 'ttest' method (regression_magnitude × alpha)."""
    run_cfg = bn.BenchRunCfg.with_defaults(run_cfg)
    bench = TtestTuning().to_bench(run_cfg)
    bench.plot_sweep(
        "Regression detection tuning — ttest",
        input_vars=["regression_magnitude", "alpha"],
...
if __name__ == "__main__":
    bn.run(example_regression_tuning_ttest)
