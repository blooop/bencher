"""Auto-generated example: Regression detection tuning — 'percentage' method (regression_magnitude × percentage_threshold)."""

import bencher as bn


class PercentageTuning(bn.ParametrizedSweep):
    """Sweep ``regression_magnitude`` × ``percentage_threshold`` for ``percentage``.
    ...
    percentage_threshold = bn.FloatSweep(
        default=5.0,
        bounds=[1.0, 20.0],
        samples=6,
        doc="Detector threshold — percent change vs baseline mean.",
    )
    ...
    def benchmark(self):
        ...
        for trial in range(self._n_trials):
            seed = (
                ...
                + int(self.percentage_threshold * 101)
            )
...
def example_regression_tuning_percentage(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    """Regression detection tuning — 'percentage' method (regression_magnitude × percentage_threshold)."""
    run_cfg = bn.BenchRunCfg.with_defaults(run_cfg)
    bench = PercentageTuning().to_bench(run_cfg)
    bench.plot_sweep(
        "Regression detection tuning — percentage",
        input_vars=["regression_magnitude", "percentage_threshold"],
...
if __name__ == "__main__":
    bn.run(example_regression_tuning_percentage)
