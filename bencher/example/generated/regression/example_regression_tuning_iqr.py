"""Auto-generated example: Regression detection tuning — 'iqr' method (regression_magnitude × iqr_scale)."""

import bencher as bn


class IqrTuning(bn.ParametrizedSweep):
    """Sweep ``regression_magnitude`` × ``iqr_scale`` for ``iqr``.
    ...
    iqr_scale = bn.FloatSweep(
        default=1.5,
        bounds=[0.5, 3.5],
        samples=6,
        doc="Detector threshold — IQR multiplier for outlier bounds.",
    )
    ...
    def benchmark(self):
        ...
        from bencher.regression import detect_iqr

        hits = 0
        for trial in range(self._n_trials):
            seed = (
                trial * 1_000_003 + int(self.regression_magnitude * 997) + int(self.iqr_scale * 101)
            )
...
def example_regression_tuning_iqr(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    """Regression detection tuning — 'iqr' method (regression_magnitude × iqr_scale)."""
    run_cfg = bn.BenchRunCfg.with_defaults(run_cfg)
    bench = IqrTuning().to_bench(run_cfg)
    bench.plot_sweep(
        "Regression detection tuning — iqr",
        input_vars=["regression_magnitude", "iqr_scale"],
...
if __name__ == "__main__":
    bn.run(example_regression_tuning_iqr)
