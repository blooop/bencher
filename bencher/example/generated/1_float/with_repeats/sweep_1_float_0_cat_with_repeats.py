...
def example_sweep_1_float_0_cat_with_repeats(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    ...
    bench.plot_sweep(
        ...
        result_vars=["time"],
        description=(
            "A 1 float + 0 categorical parameter sweep with multiple repeats per combination. "
            ...
            "calls through side effects, the statistics will be invalid. A 1D float sweep "
            "produces a line plot -- the simplest way to characterise a continuous input."
        ),
        post_description=(
