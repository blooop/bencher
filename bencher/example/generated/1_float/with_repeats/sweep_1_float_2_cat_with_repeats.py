...
def example_sweep_1_float_2_cat_with_repeats(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    """1 Float, 2 Categorical (with repeats)."""
    bench = SortAnalysis().to_bench(run_cfg)
    bench.plot_sweep(
        input_vars=["array_size", "algorithm", "distribution"],
        result_vars=["time"],
        description=(
            "A 1 float + 2 categorical parameter sweep with multiple repeats per combination. "
            ...
            "calls through side effects, the statistics will be invalid. Adding categorical "
            "variables to a float sweep creates faceted line plots -- one curve per category, "
            "making it easy to compare how each setting modifies the continuous relationship."
        ),
        post_description=(
...
if __name__ == "__main__":
    bn.run(example_sweep_1_float_2_cat_with_repeats, level=4, repeats=10)
