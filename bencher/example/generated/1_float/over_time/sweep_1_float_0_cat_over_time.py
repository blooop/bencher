...
def example_sweep_1_float_0_cat_over_time(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    ...
    for i, offset in enumerate([0.0, 0.5, 1.0]):
        ...
        res = bench.plot_sweep(
            ...
            result_vars=["time"],
            description=(
                "A 1 float + 0 categorical parameter sweep tracked over time. Setting "
                ...
                "force re-evaluation. A 1D float sweep produces a line plot -- the simplest way "
                "to characterise a continuous input."
            ),
            post_description=(
