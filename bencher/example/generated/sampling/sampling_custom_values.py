...
def example_sampling_custom_values(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    ...
    bench.plot_sweep(
        ...
        result_vars=["latency"],
        description="Custom sample values let you pick exact points to evaluate instead of a uniform sweep. Use bn.p('name', [values]) as shorthand, or Cls.param.name.with_sample_values([values]) for the explicit form. You can also use bn.p('name', samples=N) to override the number of uniform samples without listing values.",
    ...
    bench.plot_sweep(
        ...
        result_vars=["latency"],
        description="bn.p('load', samples=5) overrides how many uniformly-spaced samples are taken from the variable's bounds, without listing explicit values.",
    )
