...
def example_sweep_3_float_2_cat_no_repeats(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    ...
    bench.plot_sweep(
        ...
        description=(
            "A 3 float + 2 categorical parameter sweep with a single sample per combination. "
            ...
            "committing to longer runs. A 3D float sweep produces a volumetric "
            "representation. This is useful for visualising scalar fields in 3D parameter "
            "spaces."
        ),
        post_description=(
...
if __name__ == "__main__":
    bn.run(example_sweep_3_float_2_cat_no_repeats, level=4)
