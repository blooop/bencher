...
def example_sweep_3_float_2_cat_with_repeats(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    """3 Float, 2 Categorical (with repeats)."""
    bench = HashAnalysis().to_bench(run_cfg)
    bench.plot_sweep(
        input_vars=["key_size", "payload_size", "iterations", "algorithm", "mode"],
        result_vars=["throughput"],
        description="A 3 float + 2 categorical parameter sweep with multiple repeats per combination. Repeating measurements reveals the noise structure of your benchmark. If your function is deterministic, all repeats will be identical; if it has stochastic components, repeats let you estimate confidence intervals and distinguish signal from noise. The benchmark function must be pure -- if past calls affect future calls through side effects, the statistics will be invalid. A 3D float sweep produces a volumetric representation. This is useful for visualising scalar fields in 3D parameter spaces.",
...
if __name__ == "__main__":
    bn.run(example_sweep_3_float_2_cat_with_repeats, subsampling_divisions=4, repeats=3)
