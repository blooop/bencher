...
def example_sweep_2_float_1_cat_with_repeats(run_cfg: bn.BenchRunCfg | None = None) -> bn.Bench:
    ...
    bench.plot_sweep(
        ...
        result_vars=["ratio"],
        description=(
            "A 2 float + 1 categorical parameter sweep with multiple repeats per combination. "
            ...
            "calls through side effects, the statistics will be invalid. A 2D float sweep "
            "produces a heatmap. Additional categorical variables create faceted heatmaps, "
            "one per category combination."
        ),
        post_description=(
