...
class SweepRerunTabsND(bch.ParametrizedSweep):
    ...
    theta = bch.FloatSweep(default=1, bounds=[0, math.pi], doc="Angle parameter", units="rad", samples=3)
    ...
    def __call__(self, **kwargs):
        ...
        print(f"  Parameters: theta={self.theta:.2f}, scale={self.scale:.2f}, shape={self.shape_type}")
        
        # Log different shapes based on shape_type
        if self.shape_type == "box":
            rr.log(f"{entity_path}/shape", rr.Boxes2D(half_sizes=[self.theta * self.scale, self.scale]))
...
def example_rerun_tabs_nd(run_cfg: bch.BenchRunCfg = None, report: bch.BenchReport = None) -> bch.Bench:
    ...
    if run_cfg is None:
        run_cfg = bch.BenchRunCfg(level=2)  # Start with level 2 for manageable number of combinations
