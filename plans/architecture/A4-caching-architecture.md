...
| **B: result cache** | `cachedir/benchmark_inputs` (`result_collector.py:383-411`) | `bench_cfg.hash_persistent(include_repeats=True)` (`bench_cfg.py:784-851`) | pickled `BenchResult` | `cache_results`, `clear_cache` |
| **C: over_time history** | `cachedir/history` (`result_collector.py:412-583`) | since v1.116.0 (plans 09/14): `hash_persistent(True, include_result_vars=False)` (`bencher.py:686`) — same family as Layer B but **without** result vars, so the history survives metric-set changes and reconciles per column; repeats stay deliberately in the key so historical arrays keep their shape (`bench_cfg.py:813-815`) | pickled history record `{format, dataset, columns, retired}` (`result_collector.py:560-565`) plus a per-benchmark last-seen index | `over_time`, `clear_history`, `max_time_events`, `on_history_reset` |
...
| W4 | **Key logic scattered** across `worker_job.py`, `bench_cfg.py`, `bencher.py:679-689` (now *three* `hash_persistent` variants: result, history, sample) | nobody can answer "what invalidates what?" — worse: the `include_repeats=False` hash (`bencher.py:689`) is threaded into `WorkerJob.bench_cfg_sample_hash` and then read by **nothing** (the sample key is sorted-inputs+tag only, `worker_job.py:63`), while the stale comment at `worker_job.py:61` claims it is included |
| W5 | **History concat fragility** (largely addressed in v1.116.0, plans 09/14): renamed-vars corruption is fixed by name-in-identity keys, and incompatible (dtype/shape-mismatched) history — still discarded — now reports through `on_history_reset` (`result_collector.py:521-528`) so it can warn or raise. Remaining: NaN-vs-0 sentinel mixing after the v1.105 default flip | history loss is now observable rather than silent, but sentinel mixing still mixes regimes |
