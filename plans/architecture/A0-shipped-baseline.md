...
|---|---|---|---|
| L1.1 | Non-numeric cells are content-addressed references into a cache store | **SHIPPED** | `bencher/blob_store.py` — `materialize_blob()` (`blob_store.py:388`) writes `<cache_dir>/blobs/<sha256[:16]><ext>` and returns the **name**; `resolve_blob()` (`blob_store.py:319`) locates it (active cache dir, then the collect-time one recorded on the dataset, then a legacy cell's own path) and `load_blob()` (`blob_store.py:452`) dispatches on extension. Formats: parquet / netCDF3 / `.da.nc` / `.bin`, pickle fallback. |
| L1.2 | `ResultDataSet` payloads materialize **at collect** | **SHIPPED** | `_materialize_dataset_value()` (`result_collector.py:166`), called from `ResultCollector.store_results` at `result_collector.py:537`. The cell stores the blob name — a location-independent identity, not a path. |
