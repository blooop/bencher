...
|---|---|---|---|
| L1.1 | Non-numeric cells are content-addressed references into a cache store | **SHIPPED** | `bencher/blob_store.py` — `materialize_blob()` (`blob_store.py:388`) writes `<cache_dir>/blobs/<sha256[:16]><ext>` and returns the **name**; `resolve_blob()` locates it against cache dirs only — the one the reader named (`--cachedir` / `BENCHER_CACHE_DIR`), then its own, then the collect-time one recorded on the dataset — and `load_blob()` dispatches on extension. Formats: parquet / netCDF3 / `.da.nc` / `.bin`, pickle fallback. |
