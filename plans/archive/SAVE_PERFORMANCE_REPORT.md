> ARCHIVED: point-in-time output of `scripts/benchmark_save.py` (2026-03-23, bencher
> 1.72.2). Regenerate with `pixi run benchmark-save` rather than trusting these numbers —
> that task still writes a fresh copy to the repo root, which is now gitignored, so this
> archived copy is never overwritten.
