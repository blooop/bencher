...
class TestRunCfgMutationSafety(unittest.TestCase):
    ...
    def test_run_does_not_mutate_self_run_cfg(self):
        ...
        br.run(fidelity=3, repeats=1, cache_samples=False, over_time=True)
    ...
    def test_run_does_not_mutate_explicit_run_cfg(self):
        ...
        explicit_cfg = bn.BenchRunCfg(fidelity=5, repeats=3, run_tag="explicit")
        snapshot = {k: getattr(explicit_cfg, k) for k in explicit_cfg.param if k != "name"}

        br.run(run_cfg=explicit_cfg, fidelity=2, repeats=1)
...
class TestPerIterationIsolation(unittest.TestCase):
    ...
    def test_per_iteration_configs_are_independent(self):
        ...
        br.run(fidelity=2, max_fidelity=3, repeats=1)

        self.assertEqual(len(received_cfgs), 2)
        self.assertIsNot(received_cfgs[0], received_cfgs[1])
        self.assertEqual(received_cfgs[0].fidelity, 2)
        self.assertEqual(received_cfgs[1].fidelity, 3)

    def test_fidelity_repeats_combinations(self):
        """Multi-fidelity, multi-repeat runs must produce the exact (fidelity, repeats) pairs."""
        combos = []

        def tracking_benchmark(run_cfg, report):
            combos.append((run_cfg.fidelity, run_cfg.repeats))
        ...
        br.run(fidelity=2, max_fidelity=3, repeats=1, max_repeats=2)

        # repeats is outer loop, fidelity is inner loop
...
class TestSetupRunCfg(unittest.TestCase):
    ...
    def test_setup_run_cfg_does_not_mutate_input(self):
        ...
        original = bn.BenchRunCfg(fidelity=5, run_tag="orig")
        snapshot = {k: getattr(original, k) for k in original.param if k != "name"}

        BenchRunner.setup_run_cfg(original, fidelity=2, cache_samples=False)
...
class TestCopyElimination(unittest.TestCase):
    ...
    def test_deepcopy_count_no_explicit_run_cfg(self, mock_dc):
        ...
        br.run(fidelity=2, repeats=1)  # 1 bench_fn, 1 level, 1 repeat
    ...
    def test_deepcopy_count_with_explicit_run_cfg(self, mock_dc):
        ...
        br.run(run_cfg=bn.BenchRunCfg(run_tag="test"), fidelity=2, repeats=1)
