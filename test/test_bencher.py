...
class TestBenchRunCfgWithDefaults(unittest.TestCase):
    ...
    def test_none_creates_fresh_instance(self):
        cfg = BenchRunCfg.with_defaults(None, repeats=5, fidelity=4)
        self.assertEqual(cfg.repeats, 5)
        self.assertEqual(cfg.fidelity, 4)

    def test_defaults_applied_to_param_default_fields(self):
        cfg = BenchRunCfg()
        cfg = BenchRunCfg.with_defaults(cfg, repeats=5, fidelity=4)
        self.assertEqual(cfg.repeats, 5)
        self.assertEqual(cfg.fidelity, 4)
    ...
    def test_caller_set_fields_not_overwritten(self):
        ...
        self.assertEqual(cfg.fidelity, 4)  # default still applied

    def test_multiple_defaults_in_one_call(self):
        cfg = BenchRunCfg(fidelity=2)
        cfg = BenchRunCfg.with_defaults(cfg, repeats=3, fidelity=7, headless=True)
        self.assertEqual(cfg.repeats, 3)  # was at default, so applied
        self.assertEqual(cfg.fidelity, 2)  # caller set, so preserved
