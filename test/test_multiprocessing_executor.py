...
class TestJobPickle:
    ...
    def test_job_with_mutated_input_vars_in_bench_cfg(self):
        """BenchCfg containing with_fidelity'd sweeps must pickle inside a Job."""
        input_vars = [
            bn.StringSweep(["a", "b", "c"]).with_fidelity(3),
            bn.EnumSweep(Color).with_fidelity(2),
            bn.BoolSweep().with_fidelity(2),
            bn.IntSweep(default=0, bounds=(0, 10)).with_fidelity(3),
            bn.FloatSweep(default=0.5, bounds=(0, 1), samples=5).with_fidelity(3),
        ]
    ...
    def test_job_with_const_vars_after_with_fidelity(self):
        """const_vars containing with_fidelity'd sweeps must pickle."""
        sw = bn.FloatSweep(default=0.5, bounds=(0, 1), samples=5)
        const_sw = bn.StringSweep(["a", "b", "c"]).with_fidelity(2)
...
class TestJobMultiprocessing:
    ...
    def test_string_sweep_values_in_subprocess(self):
        job = self._make_job_with_cfg(
            [bn.StringSweep(["a", "b"]).with_fidelity(2)],
            {"label": "a"},
        )
    ...
    def test_mixed_sweep_types_in_subprocess(self):
        ...
        input_vars = [
            bn.StringSweep(["x", "y", "z"]).with_fidelity(2),
            bn.EnumSweep(Color).with_fidelity(2),
            bn.BoolSweep().with_fidelity(2),
            bn.IntSweep(default=0, bounds=(0, 5)).with_fidelity(2),
            bn.FloatSweep(default=0.0, bounds=(0, 1), samples=3).with_fidelity(2),
        ]
        job = self._make_job_with_cfg(
            input_vars,
    ...
    def test_many_string_values_in_subprocess(self):
        ...
        sw = bn.StringSweep(strings).with_fidelity(5)
...
class TestBenchCfgPickle:
    ...
    def test_bench_cfg_with_const_vars_containing_selectors(self):
        sw = bn.StringSweep(["a", "b", "c"]).with_fidelity(2)
        cfg = self._make_cfg(
...
class TestWorkerWrapperPickle:
    ...
    def test_partial_with_module_function(self):
        cfg = BenchCfg(
            input_vars=[bn.StringSweep(["a", "b"]).with_fidelity(2)],
            result_vars=[],
            const_vars=[],
            bench_name="test",
            title="test",
    ...
    def test_partial_with_parametrized_sweep_call(self):
        ...
        cfg = BenchCfg(
            input_vars=[StringOnlyConfig.param.algorithm.with_fidelity(3)],
            result_vars=[],
            const_vars=[],
            bench_name="test",
            title="test",
...
class TestFutureCacheMultiprocessing:
    ...
    def test_submit_multiple_jobs(self):
        ...
        cfg = BenchCfg(
            input_vars=[bn.EnumSweep(Color).with_fidelity(2)],
            result_vars=[],
            const_vars=[],
            bench_name="test",
            title="test",
    ...
    def test_submit_with_with_fidelity_sweep_in_cfg(self):
        """The BenchCfg inside the partial has with_fidelity'd sweeps."""
        cache = FutureCache(
            executor=Executors.MULTIPROCESSING,
            overwrite=True,
            cache_samples=False,
        ...
        input_vars = [
            bn.StringSweep(["x", "y", "z"]).with_fidelity(3),
            bn.EnumSweep(Color).with_fidelity(2),
            bn.BoolSweep().with_fidelity(2),
        ]
        cfg = BenchCfg(
...
class TestBenchMultiprocessingEndToEnd:
    ...
    def test_string_sweep_multiprocessing(self):
        ...
        res = self._run_bench(
            worker,
            [StringOnlyConfig.param.algorithm.with_fidelity(3)],
        )
    ...
    def test_enum_sweep_multiprocessing(self):
        ...
        res = self._run_bench(
            worker,
            [EnumOnlyConfig.param.algo.with_fidelity(3)],
        )
        assert res.result_samples() > 0
    ...
    def test_bool_sweep_multiprocessing(self):
        ...
        res = self._run_bench(
            worker,
            [BoolOnlyConfig.param.flag.with_fidelity(2)],
        )
        assert res.result_samples() > 0
    ...
    def test_mixed_selector_and_numeric_multiprocessing(self):
        ...
        res = self._run_bench(
            worker,
            [
                MixedSelectorConfig.param.color.with_fidelity(2),
                MixedSelectorConfig.param.label.with_fidelity(2),
                MixedSelectorConfig.param.active.with_fidelity(2),
                MixedSelectorConfig.param.weight.with_fidelity(2),
            ],
        )
        assert res.result_samples() > 0
    ...
    def test_multi_type_config_multiprocessing(self):
        ...
        res = self._run_bench(
            worker,
            [
                MultiTypeConfig.param.color.with_fidelity(2),
                MultiTypeConfig.param.label.with_fidelity(2),
                MultiTypeConfig.param.count.with_fidelity(2),
            ],
        )
        assert res.result_samples() > 0

    def test_with_samples_multiprocessing(self):
        """with_samples (not with_fidelity) under multiprocessing."""
        worker = StringOnlyConfig()
        res = self._run_bench(
            worker,
    ...
    def test_multiprocessing_with_repeats(self):
        ...
        res = bench.plot_sweep(
            "mp_repeat_sweep",
            input_vars=[StringOnlyConfig.param.algorithm.with_fidelity(2)],
            run_cfg=run_cfg,
            plot_callbacks=False,
        )
    ...
    def test_multiprocessing_with_const_vars(self):
        ...
        res = bench.plot_sweep(
            "mp_const_sweep",
            input_vars=[MixedSelectorConfig.param.weight.with_fidelity(3)],
            const_vars=[
...
class TestBenchCfgDeepcopy:
    ...
    def test_multiple_deepcopies(self):
        ...
        sw = bn.StringSweep(["a", "b", "c"]).with_fidelity(3)
        copy1 = deepcopy(sw)
...
class TestEdgeCases:
    ...
    def test_large_string_sweep(self):
        ...
        sw = bn.StringSweep(strings).with_fidelity(6)
