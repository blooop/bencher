...
def _find_holomap(obj, depth=0):
    """Recursively find the first HoloMap in a Panel layout."""
    if isinstance(obj, hv.HoloMap):
        return obj
    if depth > 10:
        return None
    if isinstance(obj, pn.pane.HoloViews) and isinstance(obj.object, hv.HoloMap):
        return obj.object
    try:
        for child in obj:
            result = _find_holomap(child, depth + 1)
            if result is not None:
                return result
    except TypeError:
        pass
...
class TestPerTimePointNoSpreadBands:
    """Item 1.6: per-time-point renders skip Spread bands for performance."""

    def test_curve_per_time_point_no_spread(self):
        """Per-time-point curve plots should not contain hv.Spread overlays."""
        benchable = FloatBench()
        res = _run_over_time(
            benchable,
            ["size", "backend"],
        ...
        plots = res.to_auto_plots()
        holomap = _find_holomap(plots)
        assert holomap is not None, "Expected a HoloMap in the plots"
        for key in holomap.keys():
            entry = holomap[key]
            if isinstance(entry, hv.Overlay):
                for el in entry.values():
                    assert not isinstance(el, hv.Spread), (
                        f"Per-time-point entry at {key} should not contain hv.Spread"
                    )

    def test_curve_aggregated_tab_has_spread(self):
        """Aggregated tab should still contain hv.Spread bands."""
        benchable = FloatBench()
        res = _run_over_time(
            benchable,
            ["size", "backend"],
        ...
        plots = res.to_auto_plots()
        # Find the Tabs that contain the aggregated tab
        found_spread = False

        def _search_tabs(obj, depth=0):
            nonlocal found_spread
            if depth > 10:
                return
            if isinstance(obj, pn.Tabs):
                for i, title in enumerate(obj._names):  # pylint: disable=protected-access
                    if "aggregated" in title.lower():
                        tab_content = obj[i]
                        # Search this tab for an Overlay containing Spread
                        _search_for_spread(tab_content, 0)
            try:
                for child in obj:
                    _search_tabs(child, depth + 1)
            except TypeError:
                pass

        def _search_for_spread(obj, depth):
            nonlocal found_spread
            if depth > 10:
                return
            if isinstance(obj, hv.Spread):
                found_spread = True
                return
            if isinstance(obj, hv.Overlay):
                for el in obj.values():
                    _search_for_spread(el, depth + 1)
                return
            if isinstance(obj, pn.pane.HoloViews):
                _search_for_spread(obj.object, depth + 1)
                return
            if isinstance(obj, hv.Element):
                return  # don't iterate on HoloViews elements
            try:
                for child in obj:
                    _search_for_spread(child, depth + 1)
            except (TypeError, NotImplementedError):
                pass

        _search_tabs(plots)
        assert found_spread, "Aggregated tab should contain hv.Spread bands"
