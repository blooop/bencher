...
def test_benchrunner_rerun_with_singleton():
    ...
    # First run
    results_first = br.run(subsampling_divisions=1, repeats=1, cache_samples=False)
    assert len(results_first) == 1

    # Second run of the same BenchRunner with the same benchable function
    results_second = br.run(subsampling_divisions=1, repeats=1, cache_samples=False)
...
def test_singleton_report_save_and_pickling():
    ...
    br.run(subsampling_divisions=1, repeats=1, cache_samples=False, save=True)
...
def test_single_argument_benchable_supported():
    ...
    results = br.run(subsampling_divisions=1, repeats=1, cache_samples=False)
