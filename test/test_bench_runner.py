...
class TestBenchRunner(unittest.TestCase):
    ...
    def test_benchrunner_default_configuration_fixed(self):
        ...
        self.assertEqual(bench_runner.run_cfg.fidelity, 2)
    ...
    #     results = bn.BenchRunner("bench_runner_test", AllSweepVars()).run(min_fidelity=1)
    #     self.assertEqual(results[0].result_samples(), 1)

    # def test_benchrunner_level_1_only(self):
    #     results = bn.BenchRunner("bench_runner_test", AllSweepVars()).run(fidelity=1)
    #     self.assertEqual(results[0].result_samples(), 1)

    def test_benchrunner_repeats(self):
        res = bn.Bench(
            "float", SimpleBenchClassFloat(), run_cfg=bn.BenchRunCfg(fidelity=2, repeats=1)
        ).plot_sweep("float")
        self.assertEqual(res.result_samples(), 2)

        res = bn.Bench(
            "float", SimpleBenchClassFloat(), run_cfg=bn.BenchRunCfg(fidelity=2, repeats=5)
        ).plot_sweep("float")
        self.assertEqual(res.result_samples(), 10)

    def test_benchrunner_unified_interface(self):
        """Test the new unified interface with fidelity/repeats and max_fidelity/max_repeats."""
        # Track what configurations are run
        executed_configs = []

        def simple_benchmark(run_cfg: bn.BenchRunCfg, report: bn.BenchReport) -> bn.BenchCfg:
            executed_configs.append((run_cfg.fidelity, run_cfg.repeats))
            bench = bn.Bench("test", SimpleBenchClassFloat(), run_cfg=run_cfg, report=report)
            return bench.plot_sweep("test")

        # Test 1: Single fidelity and repeats (no max values)
        br1 = bn.BenchRunner()
        br1.add(simple_benchmark)
        executed_configs.clear()
        results = br1.run(fidelity=2, repeats=1)
        self.assertEqual(len(results), 1)
        self.assertEqual(len(executed_configs), 1)
        self.assertEqual(executed_configs[0], (2, 1))

        # Test 2: Progressive fidelity (fidelity=2, max_fidelity=3, repeats=1)
        br2 = bn.BenchRunner()
        br2.add(simple_benchmark)
        executed_configs.clear()
        results = br2.run(fidelity=2, repeats=1, max_fidelity=3)
        # Should run at fidelity 2 and 3 = 2 results
        ...
        # Test 3: Progressive repeats (fidelity=2, repeats=1, max_repeats=2)
        br3 = bn.BenchRunner()
        br3.add(simple_benchmark)
        executed_configs.clear()
        results = br3.run(fidelity=2, repeats=1, max_repeats=2)
        ...
        # Test 4: Both progressive (fidelity=2-3, repeats=1-2) = 4 combinations
        br4 = bn.BenchRunner()
        br4.add(simple_benchmark)
        executed_configs.clear()
        results = br4.run(fidelity=2, repeats=1, max_fidelity=3, max_repeats=2)
    ...
    def test_benchruncfg_level_kwarg_deprecation(self):
        """Test that BenchRunCfg(level=5) emits DeprecationWarning and sets fidelity."""
        import warnings

        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            cfg = bn.BenchRunCfg(level=5)
            self.assertEqual(cfg.fidelity, 5)
            self.assertTrue(
                any("'level' parameter is deprecated" in str(warning.message) for warning in w)
            )
    ...
    def test_benchrunner_run_level_and_fidelity_conflict_raises(self):
    ...
    def test_benchrunner_no_name_instantiation(self):
        ...
        results = bench_runner.run(fidelity=2, repeats=1)
        self.assertEqual(len(results), 1)
    ...
    #     self.assertEqual(results[0].fidelity, 1)
    #     self.assertEqual(results[1].fidelity, 1)
    #     self.assertEqual(results[2].fidelity, 2)
    #     self.assertEqual(results[3].fidelity, 2)
    #     self.assertEqual(results[4].fidelity, 3)
    #     self.assertEqual(results[5].fidelity, 3)
    #     self.assertEqual(results[6].fidelity, 4)
    #     self.assertEqual(results[7].fidelity, 4)
    #     self.assertEqual(results[8].fidelity, 5)
    #     self.assertEqual(results[9].fidelity, 5)
    ...
    #     self.assertEqual(results[0].fidelity, 3)
    #     self.assertEqual(results[1].fidelity, 3)
    ...
    def test_benchrunner_grouped(self):
        ...
        bench_runner.add_bench(SimpleBenchClass())
        results = bench_runner.run(fidelity=2, repeats=1, grouped=True)
    ...
    def test_benchrunner_progressive_fidelity(self):
        """Test progressive fidelity runs."""
        executed = []

        def tracking_benchmark(run_cfg: bn.BenchRunCfg, report: bn.BenchReport) -> bn.BenchCfg:
            executed.append(run_cfg.fidelity)
    ...
    def test_progressive_run_auto_enables_cache(self):
        ...
        br.run(fidelity=2, max_fidelity=3)
    ...
    def test_single_fidelity_no_auto_cache(self):
        """Single-fidelity run (no max_fidelity) leaves cache_samples=False."""
        executed_cfgs = []
    ...
    def test_bench_reuse_report_cleared(self):
        """Progressive bn.run() with ParametrizedSweep produces only last fidelity's report."""
        # Progressive run from fidelity=2 to max_fidelity=3
        progressive_results = bn.run(
            SimpleBenchClassFloat, fidelity=2, max_fidelity=3, show=False, cache_samples=True
        )
        # Single-level run at the final level as a baseline for tab count
        single_results = bn.run(SimpleBenchClassFloat, fidelity=3, show=False, cache_samples=False)
        ...
        self.assertEqual(
            single_tabs,
            prog_tabs,
            "Progressive run should not accumulate tabs across fidelity levels; "
            "only the last fidelity's report should be present.",
        )

    def test_bench_reuse_cache_hits(self):
        """Progressive run where fidelity N+1 gets cache hits from fidelity N's samples."""
        results = bn.run(
            SimpleBenchClassFloat, fidelity=2, max_fidelity=3, show=False, cache_samples=True
        )
